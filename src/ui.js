/**
 * ui.js — Enhanced UI with bookmarks, compare, expandable cards, difficulty badges
 */
import { isLiked, toggleLike, isBookmarked, addBookmark, removeBookmark, isInCompare, addToCompare, removeFromCompare, getCompareList, clearCompare, addViewed } from './storage.js';
...
function freshPct(d) { return Math.max(0, Math.min(100, 100 - (d/365)*100)); }
function freshColor(d) { if(d<30) return '#10b981'; if(d<90) return '#06b6d4'; if(d<180) return '#f59e0b'; return '#ef4444'; }

// Store refs for event delegation
let _allRepos = [];
  ...
  <article class="repo-card" style="--score-color:${sc}" data-repo-id="${repo.id}">
     <div class="card-header">
      ...
      <div class="score-badge-wrapper">
        ...
        <div class="score-breakdown-tooltip">
          <h5 class="tooltip-title">Score Breakdown</h5>e">${labelScore} Breakdown</h5>
    ...
    <div class="card-details" id="details-${repo.id}">
      ...
      <div class="details-content">
        ${repo.aiPros.length?`<div class="report-section"><div class="report-title">✅ Pros</div><ul class="pros-list">${repo.aiPros.map(p=>`<li>${esc(p)}</li>`).join('')}</ul></div>`:''}
        ${repo.aiCons.length?`<div class="report-section"><div class="report-title">⚠️ Cons</div><ul class="cons-list">${repo.aiCons.map(c=>`<li>${esc(c)}</li>`).join('')}</ul></div>`:''}
...
export function renderResults(repos, container) {
  ...
  _currentPage = 1; // Reset to page 1 on new search results render
  ...
  container.addEventListener('click', (e) => {
    ...
    const repo = _allRepos.find(r => r.id == repoId);
    if (!repo) return;
...
function renderPagination(repos, wrapper, grid) {
  ...
  const start = (_currentPage - 1) * REPOS_PER_PAGE;
  ...
  for (let p = 1; p <= totalPages; p++) {
    pageButtonsHtml += `<button class="pagination-page-btn ${p === _currentPage ? 'active' : ''}" data-page="${p}">${p}</button>`;
  }
  
  wrapper.innerHTML = `
    <div class="pagination-controls-bar">
      <button class="btn-pagination-arrow" id="prev-page-btn" ${_currentPage === 1 ? 'disabled' : ''}>◀ Prev</button>
      <div class="pagination-pages-list">
        ${pageButtonsHtml}
      </div>
      <button class="btn-pagination-arrow" id="next-page-btn" ${_currentPage === totalPages ? 'disabled' : ''}>Next ▶</button>
    </div>
...
function showCompareDrawer(container) {
...
export function updateLoadingMessage(m) { const e=document.getElementById('loading-message'); if(e) e.textContent=m; }
export function updateProgress(p) { const b=document.getElementById('progress-bar'); if(b) b.style.width=p+'%'; }
